...
func projectCudBasedCheck(ctx context.Context, cud *models.WorkforceCud, m *ent.ProjectMutation) (bool, error) {
	if m.Op().Is(ent.OpCreate) {
		...
		return checkWorkforce(cud.Create, nil, &typeID, nil), nil
	}
	...
	if m.Op().Is(ent.OpUpdateOne) {
		return checkWorkforce(cud.Update, nil, projectTypeID, nil), nil
	}
	return checkWorkforce(cud.Delete, nil, projectTypeID, nil), nil
}
	...
	return privacy.ProjectMutationRuleFunc(func(ctx context.Context, m *ent.ProjectMutation) error {
		...
		if !m.Op().Is(ent.OpCreate) {
			...
			if creatorChanged {
				...
				allowed = allowed && checkWorkforce(cud.TransferOwnership, nil, projectTypeID, nil)
			}
		}
		if allowed {
			return privacy.Allow
