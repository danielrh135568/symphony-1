// Copyright (c) 2004-present Facebook All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

// Code generated by entc, DO NOT EDIT.

package formula

import (
	"time"

	"github.com/facebook/ent/dialect/sql"
	"github.com/facebook/ent/dialect/sql/sqlgraph"
	"github.com/facebookincubator/symphony/pkg/ent/predicate"
)

// ID filters vertices based on their identifier.
func ID(id int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldID), id))
	})
}

// IDEQ applies the EQ predicate on the ID field.
func IDEQ(id int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldID), id))
	})
}

// IDNEQ applies the NEQ predicate on the ID field.
func IDNEQ(id int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldID), id))
	})
}

// IDIn applies the In predicate on the ID field.
func IDIn(ids ...int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(ids) == 0 {
			s.Where(sql.False())
			return
		}
		v := make([]interface{}, len(ids))
		for i := range v {
			v[i] = ids[i]
		}
		s.Where(sql.In(s.C(FieldID), v...))
	})
}

// IDNotIn applies the NotIn predicate on the ID field.
func IDNotIn(ids ...int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(ids) == 0 {
			s.Where(sql.False())
			return
		}
		v := make([]interface{}, len(ids))
		for i := range v {
			v[i] = ids[i]
		}
		s.Where(sql.NotIn(s.C(FieldID), v...))
	})
}

// IDGT applies the GT predicate on the ID field.
func IDGT(id int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.GT(s.C(FieldID), id))
	})
}

// IDGTE applies the GTE predicate on the ID field.
func IDGTE(id int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.GTE(s.C(FieldID), id))
	})
}

// IDLT applies the LT predicate on the ID field.
func IDLT(id int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.LT(s.C(FieldID), id))
	})
}

// IDLTE applies the LTE predicate on the ID field.
func IDLTE(id int) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.LTE(s.C(FieldID), id))
	})
}

// CreateTime applies equality check predicate on the "create_time" field. It's identical to CreateTimeEQ.
func CreateTime(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldCreateTime), v))
	})
}

// UpdateTime applies equality check predicate on the "update_time" field. It's identical to UpdateTimeEQ.
func UpdateTime(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldUpdateTime), v))
	})
}

// TextFormula applies equality check predicate on the "textFormula" field. It's identical to TextFormulaEQ.
func TextFormula(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldTextFormula), v))
	})
}

// Status applies equality check predicate on the "status" field. It's identical to StatusEQ.
func Status(v bool) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldStatus), v))
	})
}

// CreateTimeEQ applies the EQ predicate on the "create_time" field.
func CreateTimeEQ(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldCreateTime), v))
	})
}

// CreateTimeNEQ applies the NEQ predicate on the "create_time" field.
func CreateTimeNEQ(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldCreateTime), v))
	})
}

// CreateTimeIn applies the In predicate on the "create_time" field.
func CreateTimeIn(vs ...time.Time) predicate.Formula {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.Formula(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.In(s.C(FieldCreateTime), v...))
	})
}

// CreateTimeNotIn applies the NotIn predicate on the "create_time" field.
func CreateTimeNotIn(vs ...time.Time) predicate.Formula {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.Formula(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.NotIn(s.C(FieldCreateTime), v...))
	})
}

// CreateTimeGT applies the GT predicate on the "create_time" field.
func CreateTimeGT(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.GT(s.C(FieldCreateTime), v))
	})
}

// CreateTimeGTE applies the GTE predicate on the "create_time" field.
func CreateTimeGTE(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.GTE(s.C(FieldCreateTime), v))
	})
}

// CreateTimeLT applies the LT predicate on the "create_time" field.
func CreateTimeLT(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.LT(s.C(FieldCreateTime), v))
	})
}

// CreateTimeLTE applies the LTE predicate on the "create_time" field.
func CreateTimeLTE(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.LTE(s.C(FieldCreateTime), v))
	})
}

// UpdateTimeEQ applies the EQ predicate on the "update_time" field.
func UpdateTimeEQ(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldUpdateTime), v))
	})
}

// UpdateTimeNEQ applies the NEQ predicate on the "update_time" field.
func UpdateTimeNEQ(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldUpdateTime), v))
	})
}

// UpdateTimeIn applies the In predicate on the "update_time" field.
func UpdateTimeIn(vs ...time.Time) predicate.Formula {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.Formula(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.In(s.C(FieldUpdateTime), v...))
	})
}

// UpdateTimeNotIn applies the NotIn predicate on the "update_time" field.
func UpdateTimeNotIn(vs ...time.Time) predicate.Formula {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.Formula(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.NotIn(s.C(FieldUpdateTime), v...))
	})
}

// UpdateTimeGT applies the GT predicate on the "update_time" field.
func UpdateTimeGT(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.GT(s.C(FieldUpdateTime), v))
	})
}

// UpdateTimeGTE applies the GTE predicate on the "update_time" field.
func UpdateTimeGTE(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.GTE(s.C(FieldUpdateTime), v))
	})
}

// UpdateTimeLT applies the LT predicate on the "update_time" field.
func UpdateTimeLT(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.LT(s.C(FieldUpdateTime), v))
	})
}

// UpdateTimeLTE applies the LTE predicate on the "update_time" field.
func UpdateTimeLTE(v time.Time) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.LTE(s.C(FieldUpdateTime), v))
	})
}

// TextFormulaEQ applies the EQ predicate on the "textFormula" field.
func TextFormulaEQ(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldTextFormula), v))
	})
}

// TextFormulaNEQ applies the NEQ predicate on the "textFormula" field.
func TextFormulaNEQ(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldTextFormula), v))
	})
}

// TextFormulaIn applies the In predicate on the "textFormula" field.
func TextFormulaIn(vs ...string) predicate.Formula {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.Formula(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.In(s.C(FieldTextFormula), v...))
	})
}

// TextFormulaNotIn applies the NotIn predicate on the "textFormula" field.
func TextFormulaNotIn(vs ...string) predicate.Formula {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.Formula(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.NotIn(s.C(FieldTextFormula), v...))
	})
}

// TextFormulaGT applies the GT predicate on the "textFormula" field.
func TextFormulaGT(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.GT(s.C(FieldTextFormula), v))
	})
}

// TextFormulaGTE applies the GTE predicate on the "textFormula" field.
func TextFormulaGTE(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.GTE(s.C(FieldTextFormula), v))
	})
}

// TextFormulaLT applies the LT predicate on the "textFormula" field.
func TextFormulaLT(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.LT(s.C(FieldTextFormula), v))
	})
}

// TextFormulaLTE applies the LTE predicate on the "textFormula" field.
func TextFormulaLTE(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.LTE(s.C(FieldTextFormula), v))
	})
}

// TextFormulaContains applies the Contains predicate on the "textFormula" field.
func TextFormulaContains(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.Contains(s.C(FieldTextFormula), v))
	})
}

// TextFormulaHasPrefix applies the HasPrefix predicate on the "textFormula" field.
func TextFormulaHasPrefix(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.HasPrefix(s.C(FieldTextFormula), v))
	})
}

// TextFormulaHasSuffix applies the HasSuffix predicate on the "textFormula" field.
func TextFormulaHasSuffix(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.HasSuffix(s.C(FieldTextFormula), v))
	})
}

// TextFormulaEqualFold applies the EqualFold predicate on the "textFormula" field.
func TextFormulaEqualFold(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EqualFold(s.C(FieldTextFormula), v))
	})
}

// TextFormulaContainsFold applies the ContainsFold predicate on the "textFormula" field.
func TextFormulaContainsFold(v string) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.ContainsFold(s.C(FieldTextFormula), v))
	})
}

// StatusEQ applies the EQ predicate on the "status" field.
func StatusEQ(v bool) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldStatus), v))
	})
}

// StatusNEQ applies the NEQ predicate on the "status" field.
func StatusNEQ(v bool) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldStatus), v))
	})
}

// HasTech applies the HasEdge predicate on the "tech" edge.
func HasTech() predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TechTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, TechTable, TechColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasTechWith applies the HasEdge predicate on the "tech" edge with a given conditions (other predicates).
func HasTechWith(preds ...predicate.Tech) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TechInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, TechTable, TechColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
		})
	})
}

// HasKpi applies the HasEdge predicate on the "kpi" edge.
func HasKpi() predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(KpiTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, KpiTable, KpiColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasKpiWith applies the HasEdge predicate on the "kpi" edge with a given conditions (other predicates).
func HasKpiWith(preds ...predicate.Kpi) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(KpiInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, KpiTable, KpiColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
		})
	})
}

// HasCounterformula applies the HasEdge predicate on the "counterformula" edge.
func HasCounterformula() predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(CounterformulaTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, CounterformulaTable, CounterformulaColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasCounterformulaWith applies the HasEdge predicate on the "counterformula" edge with a given conditions (other predicates).
func HasCounterformulaWith(preds ...predicate.CounterFormula) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(CounterformulaInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, CounterformulaTable, CounterformulaColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
		})
	})
}

// And groups list of predicates with the AND operator between them.
func And(predicates ...predicate.Formula) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s1 := s.Clone().SetP(nil)
		for _, p := range predicates {
			p(s1)
		}
		s.Where(s1.P())
	})
}

// Or groups list of predicates with the OR operator between them.
func Or(predicates ...predicate.Formula) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		s1 := s.Clone().SetP(nil)
		for i, p := range predicates {
			if i > 0 {
				s1.Or()
			}
			p(s1)
		}
		s.Where(s1.P())
	})
}

// Not applies the not operator on the given predicate.
func Not(p predicate.Formula) predicate.Formula {
	return predicate.Formula(func(s *sql.Selector) {
		p(s.Not())
	})
}
