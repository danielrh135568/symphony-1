...
func (uq *UserQuery) QueryFeatures() *FeatureQuery {
	query := &FeatureQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithFeatures tells the query-builder to eager-loads the nodes that are connected to
// the "features" edge. The optional arguments used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withOwnedWorkOrders; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.OwnedWorkOrders = []*WorkOrder{}
		}
		query.withFKs = true
	...
	if query := uq.withAssignedWorkOrders; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.AssignedWorkOrders = []*WorkOrder{}
		}
		query.withFKs = true
		query.Where(predicate.WorkOrder(func(s *sql.Selector) {
	...
	if query := uq.withCreatedProjects; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.CreatedProjects = []*Project{}
		}
		query.withFKs = true
	...
	if query := uq.withFeatures; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.FeaturesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "features": %v`, err)
		}
		query.Where(feature.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
